...
export function createMessengerReadController(options) {
  ...
  async function flush() {
    ...
    if (!result?.ok) throw new Error(result?.message || 'Не удалось отметить сообщения прочитанными.')
    lastRequested.set(conversation.name, Math.max(cmid, Number(result.up_to_cmid) || 0))
