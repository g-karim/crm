	...
	def create_custom_fields_in_remote_site(self):
		...
		try:
			...
			frappe.log_error(
				...
				f"Could not create custom fields on remote EXP ERP site: {self.erpnext_site_url}",
			)
			frappe.msgprint(
				_(
					"Could not create the EXP CRM custom fields on {0} automatically. "
					"If it is running the latest EXP ERP, enable <b>Frappe CRM Data Synchronization</b> "
					"in its CRM Settings, Otherwise check the Error Log."
				).format(self.erpnext_site_url),
				title=_("EXP ERP custom fields not created"),
				indicator="orange",
			)
