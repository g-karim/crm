...
Статус: ✅ Сделано. В EXP ERP settings убрана ссылка на `docs.frappe.io/crm/erpnext` и связанный tooltip.
...
Статус: ✅ Сделано. Иконка EXP ERP теперь задана нативно через CRM asset `/assets/crm/images/exp-sign-black.png`.
...
Статус: ✅ Сделано. Строки uploader уже проходят через `__()` в CRM, а русские значения для `Attach a File`, `Drag & Drop files here or upload from`, `Device`, `Link`, `Camera`, `Switch Camera` добавлены в общий словарь `exp_theme`.
...
Статус: ✅ Сделано. Backend layout resolver теперь генерирует переводимые placeholders через шаблоны `Select {0}...` / `Add {0}...`, а frontend `FieldLayout` и `SidePanelLayout` не сохраняют английские generated-placeholder'ы как явные пользовательские значения.
...
Статус: ✅ Сделано. Операторы фильтра используют отдельные ключи `Filter Operator ...`, оба filter-компонента возвращают переводимый fallback, а русский словарь хранится в `exp_theme`.
...
Статус: ✅ Сделано. Кнопка/tooltip настроек канбана уже проходят через `__()`, перевод `Kanban settings` добавлен в общий словарь `exp_theme`.
...
Статус: ✅ Сделано. Экран настроек воронок использует `__()` для заголовков, действий, подтверждений, сообщений ошибок, типов стадий и labels полей; недостающие русские строки добавлены в `exp_theme/translations.py`.
...
Статус: ✅ Сделано. Для новых сайтов с русской локалью дефолтная воронка и стандартные этапы создаются с русскими labels; для английских установок сохраняются прежние английские seed names. Переводы стандартных английских названий также есть в `exp_theme`.
